 ...
 */
function wavio_the_post_meta($post_id = null, $location = 'single-top')
{
    echo wavio_get_post_meta($post_id, $location); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in wavio_get_post_meta().
}
 ...
 * @return string|void
 */
function wavio_get_post_meta($post_id = null, $location = 'single-top')
{
     ...
     *
     */
    $disallowed_post_types = apply_filters('wavio_disallowed_post_types_for_meta_output', array('page'));
    ...
    if ('single-top' === $location) {
        ...
        $post_meta = apply_filters(
            'wavio_post_meta_location_single_top',
            array(
                'author',
                'post-date',
                'comments',
         ...
         */
        $post_meta = apply_filters(
            'wavio_post_meta_location_single_bottom',
            array(
                'tags',
            )
        );
    ...
    if ($post_meta && !in_array('empty', $post_meta, true)) {
        ...
        <div class="post-meta-wrapper<?php echo esc_attr($post_meta_wrapper_classes); ?>">
            ...
            <ul class="post-meta<?php echo esc_attr($post_meta_classes); ?>">
                 ...
                 *
                 */
                do_action('wavio_start_of_post_meta_list', $post_id, $post_meta, $location);
                ...
                if (post_type_supports(get_post_type($post_id), 'author') && in_array('author', $post_meta, true)) {
                    ...
                    <li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html_e('Post author', 'wavio'); ?></span>
							<?php wavio_the_theme_svg('user'); ?>
						</span>
                        <span class="meta-text">
                            ...
                            /* translators: %s: Author name. */
                                esc_html__('%s', 'wavio'),
                                '<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '">'. esc_html__('By ','wavio') . esc_html(get_the_author_meta('display_name')) . '</a>'
                            );
                            ?>
						</span>
                    </li>
                ...
                if (in_array('post-date', $post_meta, true)) {
                    ...
                    <li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html__('Post date', 'wavio'); ?></span>
							<?php wavio_the_theme_svg('calendar'); ?>
						</span>
                        <span class="meta-text">
                ...
                if (in_array('categories', $post_meta, true) && has_category()) {
                    ...
                    <li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html__('Categories', 'wavio'); ?></span>
							<?php wavio_the_theme_svg('folder'); ?>
						</span>
                        <span class="meta-text">
							<?php echo esc_html_x('In', 'A string that is output before one or more categories', 'wavio'); ?><?php the_category(', '); ?>
						</span>
                    </li>
                    <?php
                ...
                if (in_array('tags', $post_meta, true) && has_tag()) {
                    ...
                    <li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html_e('Tags', 'wavio'); ?></span>
							<?php wavio_the_theme_svg('tag'); ?>
						</span>
                        <span class="meta-text">
							<?php the_tags('', ', ', ''); ?>
						</span>
                ...
                if (in_array('comments', $post_meta, true) && !post_password_required() && (comments_open() || get_comments_number())) {
                    ...
                    <li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php wavio_the_theme_svg('comment'); ?>
						</span>
                        <span class="meta-text">
                ...
                if (in_array('sticky', $post_meta, true) && is_sticky()) {
                    ...
                    <li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php wavio_the_theme_svg('bookmark'); ?>
						</span>
                        <span class="meta-text">
							<?php esc_html_e('Sticky post', 'wavio'); ?>
						</span>
                    </li>
                    <?php
                 ...
                 *
                 */
                do_action('wavio_end_of_post_meta_list', $post_id, $post_meta, $location);
 ...
 */
function wavio_nav_menu_social_icons($item_output, $item, $depth, $args)
{
    // Change SVG icon inside social links menu if there is supported URL.
    if ('social' === $args->theme_location) {
        $svg = wavio_SVG_Icons::get_social_link_svg($item->url);
        if (empty($svg)) {
            $svg = wavio_get_theme_svg('link');
        }
...
add_filter('walker_nav_menu_start_el', 'wavio_nav_menu_social_icons', 10, 4);
 ...
 */
function wavio_is_comment_by_post_author($comment = null)
{
...
// */
//function wavio_filter_comment_reply_link( $link ) {
//
...
//}
//
//add_filter( 'comment_reply_link', 'wavio_filter_comment_reply_link' );